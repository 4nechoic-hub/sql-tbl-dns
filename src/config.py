...
class DatabaseConfig:
    ...
    @classmethod
    def from_env(cls, backend: str | None = None) -> "DatabaseConfig":
        """Load configuration from environment variables."""
        resolved_backend = backend if backend is not None else os.getenv("DB_BACKEND")
        if resolved_backend is None:
            resolved_backend = "postgresql"
